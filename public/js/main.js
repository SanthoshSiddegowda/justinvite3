jQuery(document).ready(function () {
  jQuery(".single-post .post-info").stick_in_parent({
    parent: ".wrapper",
    spacer: false,
  });
  ...
  if (jQuery(".blog-background").length) {
    if (jQuery(".grid-item").first().hasClass("p_one")) {
      ...
      jQuery(".blog-background").css({
        top: jQuery(".grid-item").first().find(".img").height() + 15,
      });
    }
  }
...
var contactFormWidthFix = function () {
  ...
  ).innerWidth(jQuery(".contact-form-form").width());
};
